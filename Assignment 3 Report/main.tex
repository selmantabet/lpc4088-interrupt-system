...
Although the button release was not explicitly defined as an interrupt, the fact that the button press ISR managed to run on a \texttt{while} loop until the button release signal is detected makes the creation of a second ISR redundant. The implementation of a second ISR would result in a nested ISR definition since the button press ISR can then only be broken with a different ISR, causing a stacked ISR call from both signals, running on top of each other. This would have been a far more complicated implementation than a simple \texttt{while} loop to be broken upon button release.
